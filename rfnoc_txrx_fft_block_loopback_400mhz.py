...
graph_connections = {
    0: {
        "tx": [("0/FFT#0", 0, "0/Radio#0", 0)],
        "rx": [("0/Radio#0", 0, "0/FFT#1", 0)],
    },
#   1: {
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="0",
        help="specifies the channels to use [default = 0].",
    )
    parser.add_argument(
        "-l",
        "--loopback",
...
def connect_graph(graph, graph_connections, channels=[], tx_streamer=None, rx_streamer=None):
    ...
    for chan_idx, chan in enumerate(channels):
        for direction in ["tx", "rx"]:
            ...
            for idx, (src_name, src_idx, dst_name, dst_idx) in enumerate(
                connections
            ):
                first = idx == 0
...
def main():
    ...
    num_chan = len(channels)
    args.args = 'addr=10.157.161.243, master_clock_rate=491.52e6'
    # Fetch the arguments
