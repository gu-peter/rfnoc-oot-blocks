...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="specifies the TX/RX radio center frequency in Hz [default = 1.0 GHz]",
    )
    ...
    for chan_idx, chan in enumerate(channels):
        ...
        for i in range(start, end):
            ...
            if mag > threshold_in:
                ...
                real = np.real(sc16_to_complex128(data_in[chan_idx][i]))
                imag = np.imag(sc16_to_complex128(data_in[chan_idx][i]))

                if (abs(real) >= 0.1) or (abs(imag) >= 0.1):
                    # filter out peaks which are below the noise level
                    print(f"Chan {chan} input peak at {i % fft_length} of {real}{imag}j")
            ...
            if mag > threshold_out:
                ...
                if (abs(real) >= 0.1) or (abs(imag) >= 0.1):
                    # filter out peaks which are below the noise level
                    print(f"Chan {chan} output peak at {i % fft_length} of {real}{imag}j")
...
def main():
    ...
    args = parse_args()
    args.args = 'addr=10.157.161.243, master_clock_rate=245.76e6'
    args.cp_list = [352, 288, 288, 288, 288, 288, 288, 288, 288, 288, 288, 288, 288, 288]
    args.freq = 0.6144e9 
    args.channels = "0"
    print_args(args)
